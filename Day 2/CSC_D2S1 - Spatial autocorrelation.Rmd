...
for (package in c("dplyr", "sf", "spdep", "elsa", "ncf", "units", "inlmisc",
                  "spatstat",  "mapview", "fpc", "GWmodel", "NbClust", 
                  "osmdata", "patchwork", "spatialreg")) {
...
vil_voronoi <- vil_voronoi %>% 
  dplyr::mutate(Morans_I = local_moran$Ii, 
                Morans_p = local_moran$`Pr(z != 0)`) # # p-value of local moran statistic. Notice that the column name has spaces, which means we need to refer to it with backticks. In finnish language keyboard, press shift + the button to the left of backspace.
...
corgram <- ncf::correlog(coordinates[,1], coordinates[,2], # point coordinates
                    ...
                    increment=100000, # distance lags, here 100 km
