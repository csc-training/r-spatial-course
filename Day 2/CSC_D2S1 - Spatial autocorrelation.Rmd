...
Spatial autocorrelation occurs when points, or their attributes are not random, but exhibit some kind of spatial relationship. Spatial autocorrelation can be positive (the attribute values are clustered), negative (values are dispersed), or there is no spatial autocorrelation (value distribution is spatially random). In other words, it measures how whether an object is similar to other objects surrounding it.
...
dmat <- 1/sf::st_distance(villages) %>% 
  units::drop_units()#unclassing because st_distance returns a 'unit' class object. Operations with 'unit' class require both operands to be of class 'unit'.
...
knn_listw <- sf::st_coordinates(villages) %>% 
    spdep::knearneigh(k=floor(250/3)) %>% 
...
```{r}
# weights based on neighborhoods
# for CSC notebooks  replace ".." with "/home/rstudio/r-spatial-course"
admin <- sf::read_sf("../Data/LAO_adm1.shp") %>%
    sf::st_transform(32648)

# vil_voronoi <- read_sf("../Data/village_voronoi.gpkg")
...
vil_voronoi <- vil_voronoi %>% 
  dplyr::mutate(Morans_I = local_moran$Ii, 
                Morans_p = dplyr::pull(local_moran,5)) # # p-value of local moran statistic. Notice that the column name has spaces, which means we need to refer to it with backticks. In finnish language keyboard, press shift + the button to the left of backspace.
...
corgram <- ncf::correlog(coordinates[,1], coordinates[,2], # point coordinates
                    ...
                    increment=30000, # distance lags, here 100 km
...
* *filter* prov_name Savannakhet in villages dataset. 
...
```{r}
vil <- read_sf("../Data/villages.gpkg") %>% 
  st_transform(32648) %>% 
  filter(prov_name == "Savannakhet")

dmat_listw <- 1/st_distance(vil) %>%
  units::drop_units() 
diag(dmat_listw) <- 0
dmat_listw <- mat2listw(dmat_listw)

moran.test(vil$LiteracyRate, dmat_listw)

ggplot(vil) + 
  geom_sf(aes(color = LiteracyRate))
...
```{r}
knn_listw <- sf::st_coordinates(vil) %>% 
    spdep::knearneigh(k=15) %>% 
    spdep::knn2nb() %>% # create neighbour list
    spdep::nb2listw(style = "W") # create weights list from neighbour object
local_moran <- spdep::localmoran(vil$LiteracyRate, knn_listw) %>% as_tibble()


vil<- vil %>% 
  dplyr::mutate(Morans_I = local_moran$Ii, 
                Morans_p = pull(local_moran,5)) # # p-value of local moran statistic. Notice that the column name has spaces, which means we need to refer to it with backticks. In finnish language keyboard, press shift + the button to the left of backspace.
plot(vil[,"Morans_I"])
plot(vil[,"Morans_p"])
```
...
```{r}
coordinates <- sf::st_coordinates(vil)

corgram <- ncf::correlog(coordinates[,1], coordinates[,2], # point coordinates
                         vil$LiteracyRate, # value for estimation
