...
There are many clustering algorithms available. In here, we have a brief look at k-means, DBSCAN and SKATER methods.
...
In addition to the spatial distribution of the clusters, we want to know what the clusters tell us. We could inspect for istance a boxplot of each cluster and each variable. 
```{r message=FALSE, warning=FALSE}
...
scale_coords <- function(sf) {
  ...
  scaledy <- scaledy/max(scaledy)*50
  
  scaledx <- coords[,2]
  scaledx <- scaledx - min(scaledx)
  scaledx <- scaledx/max(scaledx)*50
...
dmat <- scaled_dist(villages, scale_range = c(5,10))
...
villages <- read_sf("../Data/villages.gpkg") %>% 
  ...
  select(LiteracyRate, PovertyRate, IncomeDependency)

data <- st_set_geometry(villages, NULL)

clusters <- kmeans(data, 4)

villages <- mutate(villages,
...
plotdata <- tibble::add_column(data, cluster = factor(clusters$cluster))
plotdata <- tidyr::gather(plotdata, variable, value, -cluster)

p2 <- ggplot(plotdata, aes(x = variable, y = value, fill = cluster)) + 
  geom_boxplot()
