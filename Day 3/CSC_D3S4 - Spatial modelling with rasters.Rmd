...
feb_temp <- subset(temperature, subset = subset_vect)
feb_rain <- subset(rainfall, subset = subset_vect)
...
trainset <- sample(1:nrow(data), floor(nrow(data)*0.75)) %>% 
    sort()
...
cor(prediction, data$Tmon_20110701[-trainset])
...
plot(c(prediction_raster, prediction_interpolated, july2011))
```
...
* plot a scatterplot with july2011 values on x-axis, and mean error on y-axis.
