...
( select(villages, PovertyRate, LiteracyRate, prov_name) )
```
...
```{r}
read_sf("../Data/helsinki_buildings.gpkg") %>% 
  select(I_KERROSALA, D_ASHUOALA) %>% 
  mutate(proportion = D_ASHUOALA / I_KERROSALA,
         apartment_building = ifelse(proportion >= 0.5, 1, 0)) %>%
  group_by(apartment_building) %>% 
